...
- **Navigation:** a bottom tab bar (My week · Plans · Exercises) on top-level screens only; plan previews, the day screen and the player have no tabs. Tapping an exercise anywhere plays its preview straight away (no detail page: the owner found it an unnecessary step) and closing it returns there. My week lists today first; with no plan yet it says so and the app opens on Plans. Plans: goal chips (Lose weight · Get stronger · Move better), equal-height cards that are colour blocks in their goal's colour (owner chose the "bold sporty" theme for Plans) with the week's main muscles on a small tinted figure, then level · days · minutes and equipment; a plan's days open its day screen (back returns to the plan); a "create your own with AI" card at the end plus "+ Create" in the header; a plan's preview never changes your week, "Start this plan" does and asks first if you have one (past plans under "Your plans" in My week). Exercises: search, equipment chips (Bodyweight · Bands · Dumbbells · Gym; none selected = all; a chip also shows what needs less kit) and body-area chips (areas and exercises A to Z), with a line saying exactly what's shown.
