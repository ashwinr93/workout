...
| File | What it is |
|---|---|
| `index.html` | Markup only: tab screens (My week, Exercises) with a bottom tab bar, an exercise's page, day, create-a-plan, fix-a-link, player |
...
- **Navigation:** a bottom tab bar (My week · Exercises, Plans to come) on top-level screens only; the day screen, exercise pages and the player have no tabs. My week lists today first. Exercises: search, equipment chips (Bodyweight · Bands · Dumbbells · Gym; none selected = all; a chip also shows what needs less kit) and body-area chips, with a line saying exactly what's shown. Each exercise has a page with its own link (`#ex/<key>`): demo thumbnail, muscles, Focus, all cues, safety, joints, easier/harder links, "Try it with the coach". Day-screen rows open that page (with the day's amount).
